...
class BuildPyLibQCORBindingsModule(_build_ext):
    ...
    def build_cmake(self, ext):
        ...
        if 'CONDA_PREFIX' in os.environ:
            ...
            if version_info[0] == 2 or (version_info[0] == 3 and int(version_info[1]) > 7):
                cmake_args.append(
                    '-DPython_INCLUDE_DIR={}/include/python{v[0]}.{v[1]}/'.format(os.environ['CONDA_PREFIX'],
                                                                                  v=version_info))
            else:
                cmake_args.append(
                    '-DPython_INCLUDE_DIR={}/include/python{v[0]}.{v[1]}m/'.format(os.environ['CONDA_PREFIX'],
                                                                                   v=version_info))
...
with open(os.path.join(pkg_path, '../README.md'), encoding='utf-8') as f:
...
setup(
    name='qcor',
    version=version,
    description='QCOR Python package',
    ...
    keywords='C++ compiler for heterogeneous quantum-classical computing built on Clang and XACC',
    package_dir={'': os.path.relpath(pkg_path)},
    packages=find_packages(where=pkg_path),
    platforms='any',
    zip_safe=False,
    include_package_data=True,
    ext_modules=[CMakeExtension('py-qcor')],
    distclass=Distribution,

    cmdclass={
        'build_ext': BuildPyLibQCORBindingsModule,
        'clean': Clean,
        'sdist': Sdist,
        'test': Tox,
